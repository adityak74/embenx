...
class PGVectorIndexer(BaseIndexer):
    ...
    def __init__(self, dimension: int):
        ...
        self.conn_str = os.getenv("PG_CONNECTION_STRING", "postgresql://postgres:postgres@localhost:5432/postgres")
    ...
    def build_index(self, embeddings: List[List[float]], metadata: List[Dict[str, Any]]) -> None:
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute(f"CREATE TABLE {self.table_name} (id SERIAL PRIMARY KEY, embedding vector({self.dimension}), metadata JSONB)")
            
            # Prepare data
            data = [(emb, json.dumps(meta)) for emb, meta in zip(embeddings, metadata)]
            
            # Bulk insert
            execute_values(cur, f"INSERT INTO {self.table_name} (embedding, metadata) VALUES %s", data)
