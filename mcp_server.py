...
async def call_tool_impl(
    ...
    if name == "memory_add":
        ...
        return [
            TextContent(
                type="text",
                text=f"Successfully added {len(texts)} items to collection '{col_name}'. Storage: {path}",
            )
        ]
        ...
        if not os.path.exists(path):
            return [
                TextContent(
                    type="text",
                    text=f"Collection '{col_name}' not found. Path {path} does not exist.",
                )
            ]
        ...
        return [
            TextContent(
                type="text", text=f"Available collections: {', '.join(names) if names else 'None'}"
            )
        ]
