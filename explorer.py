...
selected_col_name = st.sidebar.selectbox("Select a Collection", collection_names if collection_names else ["None"])
...
with tabs[2]:
    if vectors.size > 0:
        ...
        if len(subset_vecs) >= 3:
            ...
            layer_map = [np.random.choice([0, 1, 2], p=[0.7, 0.2, 0.1]) for _ in range(len(subset_indices))]
