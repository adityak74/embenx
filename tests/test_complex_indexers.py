...
def test_scann_indexer():
    with patch("indexers.scann_indexer.scann") as mock_scann:
        mock_scann.scann_ops_pybind.builder.return_value.tree.return_value.score_ah.return_value.reorder.return_value.build.return_value.search.return_value = ([0], [0.1])
        mock_scann.scann_ops_pybind.builder.return_value.tree.return_value.score_ah.return_value.reorder.return_value.build.return_value.serialize.return_value = None
...
def test_elasticsearch_indexer():
    with patch("indexers.elasticsearch_indexer.Elasticsearch") as mock_es, \
         patch("indexers.elasticsearch_indexer.helpers") as mock_helpers:
...
def test_pgvector_indexer():
    with patch("indexers.pgvector_indexer.psycopg2") as mock_psycopg, \
         patch("indexers.pgvector_indexer.execute_values") as mock_exec:
